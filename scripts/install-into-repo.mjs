...
const forgeaiHookPath='${CLAUDE_PROJECT_DIR}/.claude/hooks/forgeai-hook.mjs';
function isForgeAIHook(hook){return Boolean(hook&&typeof hook==='object'&&((hook.command==='node'&&Array.isArray(hook.args)&&hook.args[0]===forgeaiHookPath)||(typeof hook.command==='string'&&hook.command.includes('/.claude/hooks/forgeai-hook.mjs'))));}
function hookSignature(hook){return JSON.stringify([hook?.type??null,hook?.command??null,Array.isArray(hook?.args)?hook.args:[]]);}
function withoutForgeAI(entries){return entries.flatMap(entry=>{if(!Array.isArray(entry?.hooks))return[entry];const hooks=entry.hooks.filter(hook=>!isForgeAIHook(hook));return hooks.length?[{...entry,hooks}]:[];});}
settings.hooks??={};for(const[event,entries]of Object.entries(fragment.hooks)){const existing=withoutForgeAI(settings.hooks[event]??[]);const signatures=new Set(existing.flatMap(entry=>(entry.hooks??[]).map(hookSignature)));const additions=entries.filter(entry=>!(entry.hooks??[]).some(hook=>signatures.has(hookSignature(hook))));settings.hooks[event]=[...existing,...additions];}
