...
for(const [name,value,expected] of [['sorts keys',{z:1,a:2},'{"a":2,"z":1}'],['normalizes minus zero',{n:-0},'{"n":0}'],['nested deterministic',{z:[{b:2,a:1}]},'{"z":[{"a":1,"b":2}]}']])test(`canonicalize ${name}`,()=>assert.equal(canonicalize(value),expected));
...
test('canonicalize rejects cycles',()=>{const v={};v.self=v;assert.throws(()=>canonicalize(v),/cyclic/u);});
...
for(const [path,pattern,expected] of [['src/a.js','src/**',true],['src/nested/a.js','src/**',true],['tests/a.js','src/**',false],['src/a.js','src/*.js',true],['src/nested/a.js','src/*.js',false],['src/a.ts','src/?.ts',true],['a/b/c.txt','**/*.txt',true],['a.txt','**/*.txt',true],['a/b/c.txt','a/**/c.txt',true]])test(`glob ${pattern} on ${path}`,()=>assert.equal(matchesGlob(path,pattern),expected));
